...
function getCurrentFlags() {
    const currentLangKey = i18nUtil.getCurrentLang();
    const otherLangKey = currentLangKey === 'FR' ? 'EN' : 'FR';

    return {
        selected: FLAGS.find(flag => flag.key === currentLangKey),
        unselected: FLAGS.find(flag => flag.key === otherLangKey)
    };
}
...
function createLanguageDropdown(selectedFlag, unSelectedFlag, counter) {
    ...
    const switchLangKey = unSelectedFlag.key;
    ...
    return `
        <div class="dropdown">
            ...
            <ul class="dropdown-menu ${dropdownMenuClass}" aria-labelledby="${dropdownId}">
                ...
                <li>
                    <a onclick="i18nUtil.setCurrentLang('${switchLangKey}'); location.reload();" class="dropdown-item language-switcher" data-lang-key="${switchLangKey}" href="#">
                        ${unSelectedFlag.svg} ${unSelectedFlag.text}
                    </a>
                </li>
            </ul>
        </div>
